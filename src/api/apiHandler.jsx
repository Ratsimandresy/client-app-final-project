...
const service = axios.create({baseURL: process.env.REACT_APP_BACKEND_URL, withCredentials: true});
...
export default {
    ...
    getOne(endPoint, data) {
        return service.get(`${endPoint}${data}`).then((resDb) => resDb.data).catch(errorHandler);
    },
    ...
    deleteone(endPoint, data) {
        return service.delete(endPoint, data).then((res) => res.data).catch(errorHandler);
    }
};
